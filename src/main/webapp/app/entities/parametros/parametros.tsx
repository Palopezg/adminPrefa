...
export const Parametros = (props: IParametrosProps) => {
  ...
  const { parametrosList, match, loading, totalItems } = props;
  return (
    <div>
          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('parametro')}>
                  Parametro <FontAwesomeIcon icon="sort" />
                </th>
                ...
                <tr key={`entity-${i}`}>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${parametros.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${parametros.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
      ...
      <div className={parametrosList && parametrosList.length > 0 ? '' : 'd-none'}>
...
const mapStateToProps = ({ parametros }: IRootState) => ({
  ...
  loading: parametros.loading,
  totalItems: parametros.totalItems
});
